...
import { componentLang, langPromise, langMap, LANGS } from "./langtools.js";
 ...
 */
export async function writeMultiLangJMD(mdName, folderName, callbackOnInterior, defaultCallback) {
    const date = mdName.slice(0, 10);
    const mdContentLangs = await langPromise((lang) => read(`markdown/_${folderName}/${lang}/${mdName}`));
...
export const writeMultiLang = async (filepath, pageLangs) => {
    LANGS.map((lang) => write(lang === "en" ? filepath : `${lang}/` + filepath, pageLangs[lang]));
};
...
`;
export const buildPageLangs = (titleLangs, contentLangs) => langMap(titleLangs, (lang, title) => `
<!DOCTYPE html>
<html lang="${lang}">
<head>
    <meta charset="utf-8">
...
<body>
    ${headerHTMLLangs[lang]}<div>${contentLangs[lang]}</div>${footerHTML}
</body>
</html>
`);
