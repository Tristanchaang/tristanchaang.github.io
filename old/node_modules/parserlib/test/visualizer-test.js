...
import { compile } from '../parserlib.js';
import { visualizeAsUrl } from '../internal/visualizer.js';
describe('visualizer', () => {
    ...
    it('should handle large trees', () => {
        ...
        const parser = compile(grammar, S, S.S);
        const numZeros = 500;
        const input = "".padStart(numZeros, '0');
        const tree = parser.parse(input);
        const url = visualizeAsUrl(tree, S);
        ...
        //console.error(html);
        assert(url.includes(expectedCode));
    });
    it('should handle quotes and linebreaks in terminals', () => {
        ...
        ;
        const grammar = "root ::= .*;";
        const parser = compile(grammar, Grammar, Grammar.Root);
        const tree = parser.parse("some \"double-quoted\" and \'single-quoted\' and \\backslashes\\ and \nnewline\n and \r\nWindows newline\r\n");
        const url = visualizeAsUrl(tree, Grammar);
        //console.error(url);
        assert(url.includes("nd%28%22root%22%2Cnd%28%22%27some%20%5C%22double-quoted%5C%22%20and%20%27single-quoted%27%20and%20%5C%5Cbackslashes%5C%5C%20and%20%5Cnnewline%5Cn%20and%20%5Cr%5CnWindows%20newline%5Cr%5Cn%27%22%29%2Cuu%29"));
    });
});
